...
# Charlie Public key loaded from the key file
with open("../KEYS/Charlie_public.pem", "rb") as f:
    charlie_public_key = serialization.load_pem_public_key(f.read())
# Charlie private key loaded from the key file
with open("../KEYS/Charlie_private.key", "rb") as f:
    charlie_private_key = serialization.load_pem_private_key(f.read(), password=b'Charlie')


# This function is the function that splits Charlie's number into three giving her the ability of
# sending the other 2 clients their part to compute locally the aggregation and keep one for her to compute her aggregation
...
def send_pk(pk, who) :
    ...
    # This loads the certificates which will be asked from the other parties to prove that Charlie is safe
    context = ssl.create_default_context()
    context.load_cert_chain(certfile="../CERTS/Charlie.crt", keyfile="../CERTS/Charlie.key")
...
# meaning it will send the proper splitted values to both Bob and Alice
def handle_data_transfer(Cx2,Cx3) :
    ...
    send_data(Cx2, Bob, Bob_pk)
    send_data(Cx3,  Alice, Alice_pk)
...
# Main process function, this is the function that opens Charlie's channel, here is where connection will pass througha and
# where Charlie will be able to transfer informations 
def start_Charlie_Server():
    ...
    secure_sock.bind(('localhost', 8440))
    ...
    print("Server listening on port 8440...")
    ...
    send_pk(charlie_public_key, Hospital)
    ...
    timer = threading.Timer(15, send_pk, args=(charlie_public_key,Alice))
    timer.start()

    # then send the public key to Bob
    timer = threading.Timer(20, send_pk, args=(charlie_public_key,Bob))
    timer.start()
    ...
    while True:
        ...
        if connections >= 3 and done != True:
            ...
            print("SENDING DATA FROM CHARLIE")

            Cx1,Cx2,Cx3 = three_split(patient_value)

            timer = threading.Timer(15, handle_data_transfer, args=(Cx2,Cx3))
            timer.start()
        ...
        if connections >= 3 :
            ...
            try:
                ...
                decrypted_data = charlie_private_key.decrypt(
                    encrypted_data,
                    padding.OAEP(
                        mgf=padding.MGF1(algorithm=hashes.SHA256()),
                        algorithm=hashes.SHA256(),
                ...
                # Print the current status of aggregation, just for information
        ...
        #  REMEMBER THAT NOW WE ARE STORING BASE64 STRINGS, THEY WILL NEED TO BE CONVERTED INTO OBJECTS LATER
            ...
            print("now processing the certificate from : ",client_cert['subject'][6][0][1])
