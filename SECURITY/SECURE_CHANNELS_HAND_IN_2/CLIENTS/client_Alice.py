...
# Alice Public key loaded from the key file
with open("../KEYS/Alice_public.pem", "rb") as f:
    alice_public_key = serialization.load_pem_public_key(f.read())
# Alice private key loaded from the key file
with open("../KEYS/Alice_private.key", "rb") as f:
    alice_private_key = serialization.load_pem_private_key(f.read(), password=b'Alice')
...
def send_pk(pk, who) :
    ...
    # This loads the certificates which will be asked from the other parties to prove that Alice is safe
...
def handle_data_transfer(Ax2,Ax3) :
    ...
    send_data(Ax2, Charlie, Charlie_pk)
    send_data(Ax3,  Bob, Bob_pk)

# Create a secure SSL context
context = ssl.create_default_context(ssl.Purpose.CLIENT_AUTH)
context.load_cert_chain(certfile="../CERTS/Alice.crt", keyfile="../CERTS/Alice.key")
context.load_verify_locations(cafile="../CERTS/myCA.pem")  # Trusted CA certificate (same as server)
context.verify_mode = ssl.CERT_REQUIRED 

# Main process function, this is the function that opens Alice's channel, here is where connection will pass througha and
# where Alice will be able to transfer informations 
def start_Alice_Server():
    ...
    secure_sock.bind(('localhost', 8442))
    ...
    print("Server listening on port 8442...")
    ...
    send_pk(alice_public_key, Hospital)
    ...
    timer = threading.Timer(15, send_pk, args=(alice_public_key,Bob))
    timer.start()

    # then send the public key to Charlie
    timer = threading.Timer(20, send_pk, args=(alice_public_key,Charlie))
    timer.start()
    ...
    while True:
        ...
        if connections >= 3 and done != True:
            ...
            print("SENDING DATA FROOM ALICE")

            Ax1,Ax2,Ax3 = three_split(patient_value)

            timer = threading.Timer(5, handle_data_transfer, args=(Ax2,Ax3))
            timer.start()
        ...
        if connections >= 3 :
            ...
            try:
                ...
                decrypted_data = alice_private_key.decrypt(
                    encrypted_data,
                    padding.OAEP(
                        mgf=padding.MGF1(algorithm=hashes.SHA256()),
                        algorithm=hashes.SHA256(),
        ...
        #  REMEMBER THAT NOW WE ARE STORING BASE64 STRINGS, THEY WILL NEED TO BE CONVERTED INTO OBJECTS LATER
            ...
            if client_cert['subject'][6][0][1] == 'Bob@gmail.com' :
